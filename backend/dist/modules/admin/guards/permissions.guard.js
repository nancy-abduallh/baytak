...
let PermissionsGuard = class PermissionsGuard {
    ...
    canActivate(context) {
        ...
        if (!user)
            throw new common_1.ForbiddenException('غير مصرح لك بالوصول لهذا المورد');
        if (user.role === 'super_admin')
            return true;
        const granted = new Set(user.permissions ?? []);
        const allowed = required.some((perm) => granted.has(perm));
        if (!allowed) {
            throw new common_1.ForbiddenException('لا تملك الصلاحية اللازمة لتنفيذ هذا الإجراء');
        }
        return true;
    }
};
